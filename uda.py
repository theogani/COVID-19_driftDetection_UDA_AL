...
from mmd import mmd_poly, mmd_rbf, mmd_linear
from utils import crop_data, pad_data, detect_cusum, create_uda_model
...
tqdm.pandas()
model_path = Path('Models') / 'cambridge' / 'uda_update_1'
...
detected.to_pickle(model_path / 'detected.pickle')

rest = post_development_data.sort_values('record_date').iloc[ind[-1] + 1:]
...
X_train_post, Y_train_post = np.array(detected['features'].progress_apply(lambda x: crop_data(pad_data(x))).to_list()), np.array(detected['COVID_STATUS'].to_list())
...
clbck = [ModelCheckpoint(model_path / 'model{epoch:02d}.hdf5', save_weights_only=True, verbose=1),
         EarlyStopping(monitor='domain_loss', patience=5, verbose=1, restore_best_weights=True, mode="min")]

model = create_uda_model(input_shape=(None, *X_train.shape[1:]), base_model_path=model_path.parent / 'best_model.hdf5')
h = model.fit([X_train, np.concatenate([np.repeat(X_train_post, len(X_train)//len(X_train_post), axis=0),
                                        X_train_post[np.random.choice(len(X_train_post), len(X_train)-len(X_train)//len(X_train_post)*len(X_train_post), replace=False)]])],
              Y_train,
              validation_data=(X_valid, Y_valid),
              verbose=1, batch_size=128, epochs=30,
              callbacks=clbck)
